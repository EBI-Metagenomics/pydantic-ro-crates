...
from mgnify_ro_crates.ro_crate_ui_assets_provider import RoCrateUIAssetsProvider
from tqdm import tqdm


class MotusRoCratesPreparer:
    def __init__(
        self, original_ro_crate_path, destination_folder_path, extract_multiple=False
    ):
        self.extract_multiple = extract_multiple
        self.original_ro_crate_zip_url = (
            None if extract_multiple else original_ro_crate_path
        )
    ...
    def download_ro_crate_zip_file(self):
        try:
            ...
            raise ValueError(
                f"Failed to download the zip file from {self.original_ro_crate_zip_url}: {e}"
            )
    ...
    def extract_downloaded_ro_crate_zip_file(self):
        with tarfile.open(self.downloaded_ro_crate_zip_file_path, "r:gz") as tar:
            ...
            with tqdm(
                total=len(members), desc="Extracting files"
            ) as pbar:  # Add tqdm here
    ...
    def find_multiqc_report(self):
        ...
        self.multiqc_path = glob.glob(
            os.path.join(srr_folder_path, "qc", "multiqc", "multiqc_report.html")
        )
    ...
    def find_krona_files(self):
        ...
        self.krona_files = [
            ...
            for file in glob.glob(
                os.path.join(srr_folder_path, "taxonomy", "*", "krona.html")
            )
            if not file.endswith(".DS_Store")
        ]

    def create_ro_crate_output_folder(self):
        self.ro_crate_output_folder_name = os.path.join(
            self.destination_folder_path, f"motus_{self.srr_value}"
        )
        ...
        with tqdm(total=len(self.krona_files) + 1, desc="Copying files") as pbar:
            if self.multiqc_path:
                shutil.copy2(
                    self.multiqc_path[0],
                    os.path.join(
                        self.ro_crate_output_folder_name, "multiqc_report.html"
                    ),
                )
                pbar.update(1)
            ...
            if self.krona_files:
                for krona_file in self.krona_files:
                    ...
                    krona_dest_path = os.path.join(
                        self.ro_crate_output_folder_name, f"krona_{subfolder_name}.html"
                    )
    ...
    def add_home_button_navigation_to_multiqc_report(self):
        ...
        new_multiqc_report_path = os.path.join(
            self.ro_crate_output_folder_name, "multiqc_report.html"
        )
    ...
    def create_ro_crate_preview_html(self):
        ...
        preview_content = self.ro_crate_asset_provider.generate_preview_html(
            self.srr_value,
            self.downloaded_ro_crate_zip_temp_dir,
        ...
        preview_html_path = os.path.join(
            self.ro_crate_output_folder_name, "ro-crate-preview.html"
        )
    ...
    def create_ro_crate_metadata(self):
        ...
        ro_crate_root_directory = {
            ...
            "@type": "Dataset",
            "conformsTo": {"@id": "https://w3id.org/ro/wfrun/process/0.1"},
            "creator": {"@id": "https://ror.org/02catss52", "@type": "Organisation"},
                    ...
                    metadata["@graph"].append(
                        {
                            ...
                            "@type": "Dataset",
                            "name": "krona_LSU.html",
                            "datePublished": datetime.datetime.now().strftime(
                                "%Y-%m-%d"
                            ),
                        }
                    )
        ...
        metadata["@graph"].append(
            {
                ...
                "@type": "CrateAction",
                "agent": [
                    {"@id": "https://ror.org/02catss52", "@type": "Organisation"}
                ],
                "description": "mOTUs is a containarised pipeline for profiling shotgun metagenomic data.",
                ...
                "result": [{"@id": self.original_ro_crate_path, "@type": "Dataset"}],
            }
        )
        ...
        with tqdm(
            total=len(os.listdir(self.downloaded_ro_crate_zip_temp_dir)),
            desc="Creating metadata",
        ) as pbar:
            for root, _, files in os.walk(self.downloaded_ro_crate_zip_temp_dir):
                files = [
                    filename for filename in files if not filename.endswith(".DS_Store")
                ]
        ...
        ro_crate_metadata_path = os.path.join(
            self.ro_crate_output_folder_name, "ro-crate-metadata.json"
        )
    ...
    def create_html_from_ro_crate_metadata(self):
        self.ro_crate_metadata_html = (
            self.ro_crate_asset_provider.generate_metadata_html(
                self.raw_ro_crate_metadata
            )
        )

    def zip_ro_crate_output_folder(self):
        shutil.make_archive(
            self.ro_crate_output_folder_name, "zip", self.ro_crate_output_folder_name
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "original_crate_zip_url", type=str, help="URL to the original crate zip file."
    )
    parser.add_argument("destination_folder", type=str, help="Destination folder path.")
    parser.add_argument(
        "--extract_multiple",
        action="store_true",
        help="Prepare multiple RO crates from a list of " "directories.",
    )
    args = parser.parse_args()
    preparer = MotusRoCratesPreparer(
        args.original_crate_zip_url, args.destination_folder, args.extract_multiple
    )
