...
class RoCrateUIAssetsProvider:
    def __init__(self):
        self.preview_html_styling = self.load_asset(
            "assets/css/ro-crate-preview.css", "style"
        )
        self.ro_crate_preview_script = self.load_asset(
            "assets/js/ro-crate-preview.js", "script"
        )
        self.home_button_navigation_script = self.load_asset(
            "assets/js/home-button.js", "script"
        )
        self.home_button_styling = self.load_asset(
            "assets/css/home-button.css", "style"
        )
    ...
    def generate_krona_files_list_elements(srr_folder_path):
        subfolder_links = [
            ...
            if not subfolder_name.__contains__("DS_Store")
            and not subfolder_name.startswith("._")
        ]
    ...
    def generate_metadata_html(raw_metadata):
        ...
        for entry in raw_metadata["@graph"]:
            ...
            html_string += (
                f'    <a class="data-entity-link" href="{entry_id}">{entry_id}</a>\n\n'
            )
            html_string += "    <p>\n"
